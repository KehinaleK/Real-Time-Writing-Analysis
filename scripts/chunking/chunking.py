...
def save_chunked_text_txt(file, liste_finale):


    final_text = " ".join(f"[{chunk}]" for chunk, type in liste_finale)
    
    with open(f"../../data/chunked_texts_txt/{file}", "w") as f:
        f.write(final_text)
...
def validation(folder):
    ...
    not_validated_texts = []
    for file in files:
        with open(f"{folder}/{file}", "r") as f_chunked:
            chunked_text = f_chunked.read()
            chunked_text = re.sub(r'\ǁ\d{1,3}', '', chunked_text)
            chunked_text = re.sub(r'\{\d{1,3}', '', chunked_text)
            chunked_text = re.sub(r'\<\d{1,3}', '', chunked_text)
            chunked_text = chunked_text.replace("|", "").replace("{", "").replace("}", "").replace("<", "").replace(">", "").replace("~", "").replace("[", "").replace("]", "").replace(" ", "").replace("\n", "").replace("\t", "")
            with open(f"../../data/saved_texts_txt/{file}", "r") as f_saved:
                text = f_saved.read()
                text = text.replace(" ", "").replace("\n", "").replace("\t", "")
                if text == chunked_text:
                    continue
                else:
                    print(text)
                    print(chunked_text)
...
def save_list_chunked_texts_json(dict_lists):
...
def main():
    ...
    dict_lists = {}
    original_files_number = len(files)
    for file in files:
        ...
        text_final = save_chunked_text_txt(file, liste_clean)
        print(text_final)
    
    not_validated_texts = validation("../../data/chunked_texts_txt")
    print(not_validated_texts)
    save_list_chunked_texts_json(dict_lists)
