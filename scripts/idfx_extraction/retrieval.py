...
"""
This script allows for the retrieval of real-time witing data stored in the 
resulting InputLog IDFX files. 
To run this script, two arguments are required:
    1 - Name of the idf files corpus: planification, revision, formulation.
    2 - Treshold of pauses used to divide writing bursts.
The resulting file is saved in data/tables as csv file named after the chosen corpus.
Command line (example): python3 retrieval.py -c planification -t 1.5
"""

"""
This script may not work for every IDFX corpus as some character combinations
require more specific processing. To adapt each function to these specificities, 
refer to the MANUAL.md. Even if a file is fully processed, mistakes in the csv
file may occur due to these specificites. 
"""
...
def get_burst_rows(soup: bs4.BeautifulSoup, file_name: str, thresold: int) -> List[Row]:
    ...
    or end with a control character, a deletion or a normal character) directly follows or precedes the positions
    ...
    ## The idfx structure contains "event" tags.
    ## Each element contains informations about a pressed key.
    ## Each event contains "parts" child elements.
    ## Each part element contains information about the event (the key). 
    ## The first contains information about the positions of the key in the text and the length
    ## of the text (at the time of the event)
    ## The second contains information about the value of the key (ex : L), its start time, end time
    ## and the key's name given by the InputLog software.


    # We initialise an empty list named 'running_burst'. We will construct each burst individually,
    ...
    # Punctuation keys, do not work when CTRL is pressed.
    PUNCTUATIONS = ["VK_OEM_COMMA", "VK_OEM_PERIOD", "VK_OEM_2", "VK_OEM_8"]

   
    # This variable is set to True when the user select a part of the text with the cursos.
    # The value of the selection is not appended to the CharBurst, it's replacement (the next avent)
    # will be appended instead as "is_replacement" becomes False. Once this replacement is appended,
    # the variable is set the True once again.
    is_replacement = True 

    # We start iterating over all of the event tags in an IDFX file.
    for event_tag in soup.find_all("event"):
        # May corresponds to when the user goes back to another position (maybe with the cursos.)
        # Shifts can occurs when diacritics are used. On Word, if a user types ^ and t, ^ gets its 
        # own position (ex: 670) but t does not (remains: 670). There is therefore a shift in the the position counts.
        # If a selection tag is found, the counts are actualised. t will therefore become 671. The value
        # added to the shifts variable to update positions comes back to zero in this case. 
        if event_tag["type"] == "selection":
            shifts = 0
            continue

        # The "keyboard" tag is used for events that contain information about the keys.
        # Others concern mouse or selection actions.
        if event_tag["type"] == "keyboard":
            keyboard_event_tag = event_tag
            part_tags = keyboard_event_tag.find_all("part")
            has_wordlog = False # If the part type is "wordlog"
                                # It contains our desired info.
                                # Some keys do not have them.
            ...
            for part in part_tags:
                ...
                # The part type "winlog" contains the remaining desired info.
            ...
            if not has_wordlog: # If not wordlog has been found, we might be in a window used for saving the doc (cf burst 34-35 p+s7).
                continue

            # To get the pause between our character and the next one,
            # and therefore put an end to the current burst, we need 
            # to retrieve info about the next events. 
            list_events = retrieve_next_events(keyboard_event_tag) # Retrieve next events. 
            # This info is found like in the current event. 
            for next_event_tag in list_events:
                if next_event_tag["type"] == "keyboard":
                    ...
                    for next_child in next_keyboard_part_tag_2.children:
                        ...
                        elif next_child.name == "keyboardstate":
                            next_keyboard_state = next_child.text

                # If a replacement event is found, we set accordingly to ignore or append its values.
                ...
                # If a selection event is found (not used currently but could be useful)
            ...
            # Now, we need to retrieve the value of the character that will be appended to CharBurs.
            # A value can be the content of the value tag, however, it can also be a movement or a combination
            # obtained in specific ways depending on the situation.
            if value is not None: 
                ...
                if key == "VK_LEFT":
                    ...
                    ## Some texts seem to be wrongly reconstructed because of the processing
                    ## of tabulations and line breaks. The following commented code might
                    ...
                    #     continue
                        ...
                        running_burst.append(("⌫", position, next_position))
                ## All keys are processed accordingly
                elif key == "VK_DELETE":
                    running_burst.append(("⌦", position, next_position))
                elif key == "VK_SPACE":
                    running_burst.append(("␣", position, next_position))
                    ...
                    #tabulation = True
                    running_burst.append(("↹", position, next_position))
                # Can be use for & and 1 if shift before
                    ...
                    running_burst.append(("⇪", position, next_position))
                # To process diacritics
                elif key == "VK_OEM_6": # key for ^
                    keyboardstate_value = ""
                    ...
                    if keyboardstate_value != "":
                        if keyboardstate_value in SHIFT_KEYS:
                            running_burst.append(("¨", position, next_position)) # combination to get ¨
                    if "^" in next_value: # To process ^ followed by ^
                        running_burst.append(("^", position, next_position + 1))
                        continue
                    elif len(running_burst) > 0 and running_burst[-1][0] == "^":
                        shifts += 2
                        running_burst.append(("^", position + 1, next_position + 2))
                        continue
                    else:
                        running_burst.append(("^", position, next_position + 1))
                        continue
                    ...
                    if len(running_burst) == 0:
                        ...
                        # To create accented values
                        # To find occurrences of these cases, print "id" in each if block
                        if running_burst[-1][0] in DIACRITICS and value in ["a", "e", "i", "o", "u"]:
                            if len(running_burst) > 1 and running_burst[-2][0] in DIACRITICS:
                                running_burst.append((value, position, next_position))
                            else:
                            ...
                            if running_burst[-1][0] in ["â", "ê", "î", "ô", "û"]:
                                running_burst.append((value[1], position, position + 1))
                                shifts -= 1
                        else:
                            running_burst.append((value, position, next_position))
            ...
            ## To process replacement events.
            ## Depending on the occurrence of a control character, the position of the replacement
            ## can change.
            for event_tag in list_events:
                if event_tag["type"] == "replacement":
                    if value == new_text and key not in ["VK_DELETE", "VK_LEFT", "VK_RIGHT", "VK_DOWN", "VK_UP"]:
                        running_burst.pop(-1)
                        ...
                        if key == "VK_DELETE":
                            ...
                            running_burst.append((f"↺{new_text}", replacement_start - 1, replacement_end - 1)) 
                        elif key == "VK_DOWN":
                            if len(running_burst) > 1:
                                running_burst.append((f"↺{new_text}", running_burst[-2][1], running_burst[-2][2]))
                                continue
                            else:
                                running_burst.append((f"↺{new_text}", replacement_start, replacement_end))
                        elif key == "VK_UP": 
            ...
            for next_child in next_keyboard_part_tag_2.children:
                if next_child.name == "startTime":
                    ...
                    if endTime != 0:
                        if next_keyboard_state != "":
                            pause = 0
                            pause_round = 0
                        else:
                            pause = (next_startTime / 1000) - (endTime / 1000)
                            pause_round = round(((next_startTime / 1000) - (endTime / 1000)), 2)
                        ...
                        continue
                        # Some SHIFT keys have "0" as their endTime. Same for control characters.
                    ...
                    if pause_round > thresold:
                        ...
                        burst_row = Row(id=id, control=control, tool=tool, n_burst=n_burst, 
                                          ...
                                          docLen=docLen, categ="", charBurst=charBurst, pauses=pauses, ratio=ratio)
                        ...
                        pauses.append(round(pause, 5))
                        continue
                        # If the pause is not greater than the specified thresold, we keep on building the current burst.

   # Ensures that the last burst is appended to raw_rows.
    if running_burst:
        n_burst += 1
        burstDur = round(((endTime / 1000) - burstStart), 2)
...
def retrieve_next_events(keyboard_event_tag: bs4.element.Tag) -> bs4.element.Tag:
        """This function allows to retreive the events following the event tag being processed.
        These events help with getting the pause between the current key and the next one and furthermore
        can be essential to create combine characters such as accented ones. Others might help to process
        replacement and selection events.
        
        Parameter(s):
        keyboard_event_tag = BeautifulSoup bs4.element.Tag: complete currently processed event_tag
        Returns:
        list_events = List[BeautifulSoup bs4.element.Tag]: list of next events following the tag.
        """
...
def divide_bursts(raw_rows : List[Row]) -> Bursts:
    """This function divides the current row (corresponding to a burst initially) to multiple rows.
    If a control key is used, another row (for the same burst) is created with a new StartPos. Since
    we target the reconstruction of each test using each row sequentially, dividing the burst is essential
    to keep track of the position of the user.
    
    Parameter(s):
        raw_rows =  List[Row objects]: list of row objets (with each row = one burst)
    Returns:
        bursts = Burst: list of Burst objects (where each Burst is a list of rows objects).
    """

    ## We iterate through each row object. 
    bursts = Bursts()
    for burst in raw_rows:
        ...
        for i in range(len(burst.charBurst)):
            ...
            # If the row contains a control character
            # We split it to get separate rows within the burst object.
            if curr_burst[0][0] in  ["⇦", "⇨", "⇧", "⇩", "⏎", "↹", "⌫", "⌦", "↺"]:
                ...
                burst_part =  Row(id=burst.id, control=burst.control, tool=burst.tool, 
                                    ...
                                    charBurst=curr[0], pauses=burst.pauses, ratio = burst.ratio)
                list_burst.rows.append(burst_part)
            # If there isn't a control character, ....
            else:
                ...
                burst_part = Row(id=burst.id, control=burst.control, tool=burst.tool, 
                                ...
                                pauses = burst.pauses, ratio = burst.ratio)
                list_burst.rows.append(burst_part)
                list_curr.clear()
...
def get_categories(bursts):
    ...
    """
    This function allows to categorise each burst based on the position of its characters.
            ...
            # For the others, if a deletion or a insertion was made in the intervals of the preceeding burst, it is an edge revision.
                ...
                for burst_intervals in list_intervals[:-1]:
                    for interval in burst_intervals:
                        ...
                        if row.charBurst in ["⌫", "⌦"]:
                            if interval[1] <= int(row.posStart) and int(row.posStart) <= interval[2]:
                                row.categ = "R"
                        else:
                            if int(row.posStart) >= interval[1] and int(row.posStart) < interval[2]:
                                row.categ = "R"
                # If it was not made in any of the intervals, it is a production.
                if row.categ == "" or row.charBurst == "↺":
                    row.categ = "P"
        list_intervals.append(intervals_burst)      
...
def create_csv(table_path, list_all_bursts):
    ...
    with open(f"{table_path}.csv", "w") as file:
        ...
        structure.writerow(["ID", "control", "tool", "n_burst", "burstStart", "burstDur", "pauseDur", "cycleDur", "burstPct", "pausePct", "totalActions", "totalChars", "finalChars", "totalDeletions", "innerDeletions", "posStart", "posEnd", "docLen", "categ", "charBurst", "pauses", "ratio"])
            ...
            for Burst in bursts.bursts: 
                for row in Burst.rows:
                    if row.charBurst != "⇦" and row.charBurst != "⇨" and row.charBurst != "⇧" and row.charBurst != "⇩" and row.charBurst != "∅" and row.charBurst != "⇪" and row.charBurst != "" and row.charBurst != "⇲":
                        structure.writerow([row.id, row.control, row.tool, row.n_burst, row.burstStart, row.burstDur, row.pauseDur, row.cycleDur, row.burstPct, row.pausePct, row.totalActions, row.totalChars, row.finalChars, row.totalDeletions, row.innerDeletions, row.posStart, row.posEnd, row.docLen, row.categ, row.charBurst, row.pauses, row.ratio])
