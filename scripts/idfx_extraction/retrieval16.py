...
def get_idfx_files(corpus_path: str) -> List[str]:
    ...
    files_list = sorted(files, key=lambda x: (x[1], int(x[3:x.find('.idfx')]))) #à utiliser pour tout, utiliser la liste d'en dessous pour Maddie
    #files_list = ['P+S1.idfx', 'P+S2.idfx', 'P+S3.idfx', 'P+S4.idfx', 'P+S5.idfx', 'P+S6.idfx']
                  #'P+S7.idfx', 'P+S12.idfx','P+S14.idfx', 'P+S16.idfx']
    #, 'P+S2.idfx', 'P+S3.idfx', 'P+S4.idfx', 'P+S5.idfx', 'P+S6.idfx', 'P+S7.idfx', 'P+S12.idfx', 
     #            'P+S14.idfx', 'P+S16.idfx', 'P-S1.idfx', 'P-S2.idfx', 'P-S5.idfx', 
      #           'P-S6.idfx', 'P-S7.idfx', 'P-S8.idfx', 'P-S11.idfx', 'P-S12.idfx', 'P-S13.idfx', 'P-S15.idfx']
    return files_list
...
def get_burst_rows(soup: bs4.BeautifulSoup, file_name: str, thresold: int) -> List[Row]:
    ...
    for event_tag in soup.find_all("event"):
        if event_tag["type"] == "insert":
            print(file_name)
            # shifts = 0
            # insert_event_tag = event_tag
            # part_tags = insert_event_tag.find_all("part")
            # for part in part_tags:
            #     if part["type"] == "wordlog":
            #         for child in part:
            #             if child.name == "position":
            #                 position = int(child.text)
            #             elif child.name == "after":
            #                 new_value = child.text
                
            #         running_burst.append((f"↺{new_value}", {position}, {next_position}))
            #         continue
        ...
        if event_tag["type"] == "keyboard":
            ...
            if value is not None: # Never found a None value but we never know. Might need to add a else condition if you find one.
                ...
                if key == "VK_LEFT":
                    ...
                    if "^" in next_value:
                        ...
                        continue
                    elif len(running_burst) > 0 and running_burst[-1][0] == "^": # pour les cas ou ^^e comme dans ps+12
                        shifts += 2
                        running_burst.append(("^", position + 1, next_position + 2))
                        continue
                    else:
                        running_burst.append(("^", position, next_position + 1))
                        continue # je continue sinon on peut pas choper les cas ou ^^e ou les accents sont pas dans les mm running bursts
                else:
                    if len(running_burst) == 0:
                        ...
                        if running_burst[-1][0] in DIACRITICS and value in ["a", "e", "i", "o", "u"]:
                            ...
                            continue # je continue pour les cas ou ^m suit un ê pour pouvoir arranger ça...
            ...
            for event_tag in list_events:
                if event_tag["type"] == "replacement":
                    if value == new_text and key not in ["VK_DELETE", "VK_LEFT", "VK_RIGHT", "VK_DOWN", "VK_UP"]: #dans le cas de hello (test du 31/07..) on le remplace par lui même mais avec indication de remplacement
                        running_burst.pop(-1)
                        ...
                        if key == "VK_DELETE":
                            ...
                            running_burst.append((f"↺{new_text}", replacement_start, replacement_end)) #Dans P+S1
                            continue
                        elif key == "VK_RIGHT":
                            ...
                            continue # Dans P+S3
                        elif key == "VK_DOWN": #dans Ps+11... et d'autres problèmes ! 
                            if len(running_burst) > 1:
                        ...
                        elif key == "VK_UP": #dans Ps+12, quand on a un UP, le remplacement ne fait rien apparemment... ALORS LA
                            continue
                        else:
                            #print(event_tag)
                            running_burst.append((f"↺{new_text}", replacement_start, replacement_end))
            ...
            for next_child in next_keyboard_part_tag_2.children:
                if next_child.name == "startTime":
                    ...
                    if endTime != 0:
                        ...
                        continue
                        # Some SHIFT keys have "0" as their endTime, don't ask me why. SAME FOR ARROWS
   ...
   # our running burst.
    for row in raw_rows:
        print(len(row.charBurst), len(row.pauses))
    return raw_rows 
