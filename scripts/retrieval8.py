            ...
            if value is not None: # Never found a None value but we never know. Might need to add a else condition if you find one.
                ...
                if key == "VK_LEFT":
                    ...
                    if len(running_burst) == 0:
                        ...
                        # elif running_burst[-1][0] in DIACRITICS and value not in ["a", "e", "i", "o", "u",""] and "^" in value:
                        #     print("celui là a un accent celou")
                        #     shifts += 1
                        #     running_burst.pop(-1)
                        #     accent = value[0]
                        #     letter = value[1]
                        #     running_burst.append((accent, position, position + 1))
                        #     running_burst.append((letter, position + 2, next_position + 2))
                        else:
                            running_burst.append((value, position, next_position))
