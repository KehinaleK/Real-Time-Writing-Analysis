        ...
        if index == 0:
            ...
            if "⌫" in current_string_list or "⌦" in current_string_list:
                ...
                if len(current_string_list) > 1:
                    ...
                    pause_symbole = get_pause_category(pause_dur)
                    list_text.insert(current_posStart, (pause_symbole, string[0], ""))
                    current_posStart += 1
                ...
                insertions(list_text, category, current_posStart, string, first_row, pause_dur)
...
def get_pause_category(pause_dur):

    
    cat_1 = (" cat_1 ", 1.5099999904632568, 1.7999999523162842)
    cat_2 = (" cat_2 ", 1.7999999523162842, 2.2200000286102295)
    cat_3 = (" cat_3 ", 2.2200000286102295, 2.859999895095825)
    cat_4 = (" cat_4 ", 2.859999895095825, 4.139999866485596)
    cat_5 = (" cat_5 ", 4.139999866485596, 5.980000019073486)
    outliers = (" outlier ", 5.980000019073486, 100)

    list_cat = [cat_1, cat_2, cat_3, cat_4, cat_5]

    for category in list_cat:
        if category[1] <= pause_dur < category[2]:
            return category[0]
...
def insertions(list_text, category, current_posStart, string, first_row, pause_dur):

    pause_symbole = get_pause_category(pause_dur)

    pause_symbole = pause_symbole if first_row == True else ""

    if category == "R":
        if len(string) <= 1:
            symbol_l = "<"
            symbol_r = ">"
            list_text.insert(current_posStart, (f"{symbol_l}{pause_symbole}", string[0], symbol_r))
        else:
            symbol_l = "{"
            symbol_r = "}"

            list_text.insert(current_posStart, (f"{symbol_l}{pause_symbole}", string[0], ""))
            current_posStart += 1
...
def validation(folder):
    ...
    for file in files:
        with open(f"{folder}/{file}", "r") as f_behaviours:
            ...
            text_behaviours = text_behaviours.replace(" cat_1 ", "").replace(" cat_2 ", "").replace(" cat_3 ", "").replace(" cat_4 ", "").replace(" cat_5 ", "").replace("None", "")
...
def main():
    ...
    folder = "../../data/reconstructed_texts_behaviours_learning"
    for id, group in grouped:
        print(f"Text {id} is being constructed...")
        df_updated = mark_revisions_pipes(group)
        text = reconstruction(df_updated, debug)
        text = text.replace("|", "").replace("{", "").replace("}", "").replace("<", "").replace(">", "").replace("~", "").replace("None", "")
