        ...
        if index == 0:
            ...
            if "⌫" in current_string_list or "⌦" in current_string_list:
                ...
                if current_posEnd >= len(list_text) and len(list_text) != 0:
                    ...
                    deletions(current_posEnd, current_posStart, category, list_text, previous_start, previous_end)
                    previous_start = current_posStart
                    previous_end = current_posEnd
                ...
                num_revision = insertions(list_text, category, current_posStart, string, first_row, previous_start, previous_end, num_revision, previous_string)
...
def deletions(current_posEnd, current_posStart, category, list_text, previous_start, previous_end):
    if current_posEnd == 0:
        if len(list_text) != 0:
        ...
        if category == "R":
            if previous_end == current_posStart:
                list_text[current_posEnd - 1] = (list_text[previous_end-1][0], list_text[previous_end - 1][1], "~" + list_text[previous_end - 1][2])
                return
            else:
                history = list_text[current_posEnd - 1][2] + "<~>"
        else:
            history = list_text[current_posEnd - 2][2] + "~"

        new_tuple = ("", list_text[current_posEnd - 2][1], history)
        list_text[current_posEnd - 2] = new_tuple

def insertions(list_text, category, current_posStart, string, first_row, previous_start, previous_end, num_revision, previous_string):

    pipe = "|" if first_row == True and category != "R" else "∥" if first_row == True and category == "R" else ""
    indice = str(num_revision)
    if category == "R":
        if len(string) <= 1:
            symbol_l = "<"
            symbol_r = ">"
            if previous_string != "↺":
                list_text[previous_end - 1] = (list_text[previous_end-1][0], list_text[previous_end - 1][1], list_text[previous_end - 1][2] + pipe + indice)
            list_text.insert(current_posStart, (f"{symbol_l}{indice}", string[0], symbol_r))
            num_revision += 1
        else:
            symbol_l = "{"
            symbol_r = "}"
            ...
            list_text.insert(current_posStart, (f"{symbol_l}{indice}", string[0], ""))
            current_posStart += 1
...
def validation(folder):
    ...
    for file in files:
        with open(f"{folder}/{file}", "r") as f_behaviours:
            ...
            save = text_behaviours
            text_behaviours = re.sub(r'\ǁ\d{1,3}', '', text_behaviours)
            text_behaviours = re.sub(r'\{\d{1,3}', '', text_behaviours)
            text_behaviours = re.sub(r'\<\d{1,3}', '', text_behaviours)
