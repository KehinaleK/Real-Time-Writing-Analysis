...
def get_len(bursts):
    ...
    for index in range(len(bursts.bursts)):
        ...
        for i in range(count_start, len(burst.rows)):
            ...
            elif row_charBurst != "⌫" and all(char not in NON_LETTERS_OR_DEL for char in row_charBurst):
...
def first_deletions(start, burst, first_row_charBurst):
    ...
    for i in range(start, len(burst.rows) - 1):
        if burst.rows[i].charBurst == "⌫":
            burst_deletions += 1
        else:
            break
